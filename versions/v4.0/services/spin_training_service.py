...
class SpinTrainingService(BaseTrainingService):
    ...
    async def start_training(self, user_id: int, scenario_config: Dict[str, Any]) -> str:
        ...
        """
        try:
            # ВАЖНО: Сначала очищаем сессию перед началом новой тренировки
            # Это гарантирует, что старые данные (вопросы, счетчики, clarity_level) не переносятся в новую тренировку
            self.reset_session(user_id, scenario_config)
            logger.info(f"Session reset for user {user_id} before starting new training")

            # Получаем данные пользователя (после сброса сессии)
            ...
            client_case_raw = self.case_generator.build_case_direct(case_data)

            # Обертываем кейс в формат с объяснением SPIN-вопросов
            from modules.payments.messages import CASE_WITH_SPIN_EXPLANATION
            client_case = CASE_WITH_SPIN_EXPLANATION.format(
                case_description=client_case_raw
            )

            # Сохраняем сгенерированный кейс (сохраняем raw версию для внутреннего использования)
            session['client_case'] = client_case_raw
